function [x,rho]=gmres3(values,colid,rowptr,rows,b,xo,m)
...
rho= sqrt(rho)/normro;
for i=1:m
    ortho(i,1)= dot(V(:,i), v(:,1))
end
figure(1)
semilogy(1:j,res(1:j))
xlabel('Number of Iterations');
ylabel('Relative Residual (semilog)');
title('Full GMRES');
figure(2)
x=1:m;
semilogy(x,ortho)
xlabel('Number of Iterations');
ylabel('Dot product (semilog)');
title('Full GMRES orthogonaity');
figure(3)
x=1:m;
plot(x,ortho)
xlabel('Number of Iterations');
ylabel('dot product linear');
title('Full GMRES orthogonality');
end
